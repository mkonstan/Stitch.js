...
const RELEASE_CANDIDATE_PATH = process.env.STITCH_RC_PATH || "dist/stitch.assembled.js";
const releaseCandidateAbsPath = path.join(ROOT, RELEASE_CANDIDATE_PATH);

if (!fs.existsSync(releaseCandidateAbsPath)) {
  throw new Error(`Release candidate artifact not found: ${releaseCandidateAbsPath}`);
}

const releaseCandidateSource = fs.readFileSync(releaseCandidateAbsPath, "utf8");
...
async function installReleaseCandidateRoute(page) {
  // Swap any test page request for stitch.js with assembled RC code.
  await page.route("**/stitch.js", async (route) => {
    await route.fulfill({
      status: 200,
      contentType: "application/javascript",
      body: releaseCandidateSource,
    });
  });
}
...
async function runSuite(browser, file) {
  ...
  await installReleaseCandidateRoute(page);
...
async function main() {
  ...
  try {
    ...
    const report = {
      ...
      artifact: RELEASE_CANDIDATE_PATH,
      suites: suiteResults,
      targeted,
    };
